function [X,X1] = CircleChange(x,c,r,sigma,x1)
%闭回路调整法
X=zeros(r,c);
X1=zeros(r,c);
...
for i=1:row3
    if min_x1==0
        ...
        x1(single(i,1),single(i,2))=x1(single(i,1),single(i,2))+min_x1;
        single(i,3)=x1(single(i,1),single(i,2));
    else
        x(single(i,1),single(i,2))=x(single(i,1),single(i,2))+min_x;
        x1(single(i,1),single(i,2))=x1(single(i,1),single(i,2))+min_x;
        single(i,3)=x(single(i,1),single(i,2));
    end
end
%将偶数编号数字格的运量减去min_x
for i=1:row4
    x(double(i,1),double(i,2))=x(double(i,1),double(i,2))-min_x;
    x1(double(i,1),double(i,2))=x1(double(i,1),double(i,2))-min_x1;
    double(i,3)=x(double(i,1),double(i,2));
end
if any(double(:,3)==0)
    x1(double(1,1),double(1,2))=0;
    for i=2:row4
        if double(i,3)==0
            x(double(i,1),double(i,2))=pi-3;
            
        end
    end
end
for i=1:row3
    if single(i,3)==pi-3+min_x
        x(single(i,1),single(i,2))=min_x;
    end
end
if x1(r0,c0)==0
    x(r0,c0)=pi-3;
